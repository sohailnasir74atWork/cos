...
import { ref, update, get, onChildAdded, onChildChanged, onChildRemoved, onValue, off } from '@react-native-firebase/database';
...
export const ChatStack = ({ selectedTheme, setChatFocused, modalVisibleChatinfo, setModalVisibleChatinfo }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeAdded = onChildAdded(userChatsRef, handleChildChange);
    const unsubscribeChanged = onChildChanged(userChatsRef, handleChildChange);
    const unsubscribeRemoved = onChildRemoved(userChatsRef, handleChildRemoved);
  ...
  useEffect(() => {
    ...
    const unsubscribeValue = onValue(userGroupsRef, (snapshot) => {
      try {
        if (!snapshot.exists()) {
          setGroups([]);
