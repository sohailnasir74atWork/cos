...
const ChatScreen = ({ selectedTheme, bannedUsers, modalVisibleChatinfo, setChatFocused,
    ...
    async (reset = false) => {
      try {
        ...
        const messageQuery = reset
          ? query(chatRef, orderByKey(), limitToLast(limitSize))
          : query(chatRef, orderByKey(), endAt(lastLoadedKey), limitToLast(limitSize));
  ...
  useEffect(() => {
    ...
    const messageQuery = query(chatRef, limitToLast(1));
    const unsubscribeAdded = onChildAdded(messageQuery, (snapshot) => {
      if (!snapshot || !snapshot.key) return;
      const data = snapshot.val();
      if (!data || typeof data !== 'object') return;
  ...
  const handleSendMessage = async (replyToArg, trimmedInputArg, fruits, emojiUrl) => {
    ...
    const currentMessage = {
      text: trimmedInput,
      fruits: hasFruits ? JSON.stringify(fruits.sort((a, b) => String(a?.id || '').localeCompare(String(b?.id || '')))) : null,
      emoji: emojiUrl || null,
    };
