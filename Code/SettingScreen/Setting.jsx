...
const EditProfileDrawerContent = ({
  ...
  // FIX: User can change profile image ANYTIME. Cooldown only applies to Name.
...
export default function SettingsScreen({ selectedTheme }) {
  ...
  // Fix: Use direct image path for CoS items in avatar list
  const petAvatarOptions = useMemo(() => {
    ...
    return parsedValuesData
      ...
      .map(item => {
        // FIX: Use item.image directly for CoS items
        return {
  ...
  const handleSaveChanges = async () => {
    ...
    // ✅ Only check cooldown if displayName is being changed (Avatar/Bio are free)
    if (displayNameChanged && user?.lastProfileEditAt) {
      ...
      if (!isNaN(lastEditTimestamp)) {
        ...
        if (daysSinceLastEdit < PROFILE_EDIT_COOLDOWN_DAYS) {
          showErrorMessage(
            'Edit Cooldown',
            `You can only edit your display name once every ${PROFILE_EDIT_COOLDOWN_DAYS} days. Please try again in ${daysRemaining} day${daysRemaining === 1 ? '' : 's'}.`
          );
          return;
        }
      }
    ...
    try {
      ...
      // Only update lastProfileEditAt if displayName changed
      const updateData = {
      ...
      if (displayNameChanged) {
        updateData.lastProfileEditAt = now; // ✅ Store timestamp only when name changes
      }
